...
// Home membership, ownership transfer and Home action requests are extracted
// to domains/home so index.js remains a composition root.
...
// ================= HOME ACTION REQUEST DOMAIN =================
// Owns Alarm Pause, physical-siren and Alarm Incident action request
// validation, authorization, result cleanup and Firebase mutations.
const homeActionRequestDomain = createHomeActionRequestDomain({
  db,
  deviceId: DEVICE_ID,
  lastNotificationMap,
  getCachedAccountData,
...
if (!registerBackendFinalizer({
  ...
  handler: stopHomeActionRequestRuntime,
})) {
  throw new Error(
    "Duplicate backend lifecycle finalizer: home_action_request_runtime",
  );
}
